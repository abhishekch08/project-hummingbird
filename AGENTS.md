...
- Commit/publish reviewable work. Keep `main` as the reviewed baseline. CH05 has a published literature gate and conditional timing model; physical timing is on hold. CH06 has a published survey and candidate host model, with ABI/physical link on hold. CH07 has a merged source survey and passing conditional numerical model, but is paused at topology/block-spec freeze for approved electrode/quality/PDK/safety inputs; CH08 and later depend on its unfulfilled gate. CH03 battery and CH04 host/memory physical feasibility remain on hold pending measured inputs.
